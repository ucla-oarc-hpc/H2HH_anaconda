...
- Conda is also a package manager
  - Many packages/software can be install via Conda's repository 
...
You maybe familiar with using Anaconda on your local machine. 
...
:::
::: {.fragment}
When conda is install, it creates an conda env named `base` that you may see when running `conda env list`

- Located in the central anaconda installation path can **CANNOT** be modified by users
...
Once your conda env is activate, you can install more packages with
...
Once you create you env, you don't need to create it again.
...
While Hoffman2 already has Anaconda install, you may need to install Anaconda on a separate machine or another HPC resource.
